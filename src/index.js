...
app.patch('/todo/:user_id/:todo_id', async (req, res) => {
  ...
  try {
    ...
    const updatedTodos = await pool.query('UPDATE todos SET todo_description = ($1), todo_done = ($2) WHERE todo_id = ($3) RETURNING *', [data.description, data.done, todo_id])
    return res.status(200).send(updatedTodos.rows)
  } catch (error) {
    return res.status(400).send(error)
  }
})
